...
def on_press(key):
    try:
        key_press = key.char
        if key_press == TASK_CHOICES[WIRE]["KEY_PRESS"]:
            wire()
        elif key_press == TASK_CHOICES[CARD]["KEY_PRESS"]:
            card()
        elif key_press == TASK_CHOICES[REACTOR_NUMBERS]["KEY_PRESS"]:
            reactor()
        elif key_press == CLOSE:
            # Stop listen
            return False
